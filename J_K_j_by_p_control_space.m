%该代码可以通过调整pJ和pK来寻找领导型耦合的swarmalator集体行为状态
%This code can be used to find the collective behaviors of swarmalators with dominant coupling by adjusting pJ and pK.

clc,clear,close all
% 定义全局变量 | Define global variables
global omega K N J self_velocity d

% 振子总数,固有频率，自推进速度 | Number of oscillators, natural frequency, and self-propulsion velocity
N=500;
omega = zeros(N,1); 
self_velocity = zeros(2*N,1);
...
% J和K的双δ分布 | Double delta distribution for J and K
...
initial_space_theta = [initial_space, initial_theta]; % 将两个向量拼接为一个向量 | Concatenate two vectors

% 初始每个振子的空间位置向量和相位向量 | Initial spatial velocity and phase velocity vectors
space_x_velocity = zeros(N,1);
space_y_velocity = zeros(N,1);
theta_velocity = zeros(N,1);
...
% 所有振子中心化 | Centralize all oscillators
zx_x = (1/N)*sum(space_x_velocity);
zx_y = (1/N)*sum(space_y_velocity);

space_x_velocity1 = space_x_velocity - zx_x;
space_y_velocity1 = space_y_velocity - zx_y;

% 获得最后时刻的所有振子空间角 | Get spatial angles of all oscillators at last moment
space_angle = atan(space_y_velocity1./space_x_velocity1) + ((sign(space_x_velocity1)-1)/2) * pi;
...
function dtheta_and_dspace_dt = swarmalator_ode(t,space_theta)
    ...
    for i=1:N
        ...
        vq_space_x = ((space_x-space_x(i))./distance).*(1+J.*cos(theta-theta(i)))-(space_x-space_x(i))./(distance.^2);
        vq_space_y = ((space_y-space_y(i))./distance).*(1+J.*cos(theta-theta(i)))-(space_y-space_y(i))./(distance.^2);
        vq_theta = K.*(sin(theta-theta(i)))./distance;
        ...
        dtheta_and_dspace_dt(2*i-1) = self_velocity(2*i-1)+(1/N)*sum(vq_space_x);
        dtheta_and_dspace_dt(2*i) = self_velocity(2*i)+(1/N)*sum(vq_space_y);
        dtheta_and_dspace_dt(i+2*N) = omega(i) + (1/N)*sum(vq_theta);
    end
    disp(t);
end
